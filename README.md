...
Render es ideal porque **NO requiere tarjeta de crédito** en el free tier y soporta SQLite con discos persistentes.

Ver guía completa en **[RENDER_DEPLOY.md](RENDER_DEPLOY.md)** con todos los pasos detallados.

#### Resumen rápido:

1. Ve a [render.com](https://render.com) y crea cuenta con GitHub
2. **New +** → **Web Service** → Conecta tu repo `nature-app`
3. **Build Command:**
   ```bash
...
6. **Persistent Disk:**
   ...
   - Size: 1 GB
7. Selecciona plan **Free** y haz clic en **Create Web Service**
...
**⚠️ Nota:** La app se "duerme" tras 15 min sin tráfico (primera carga lenta).

### Otras opciones:
- **Ngrok** - Túnel público temporal desde tu PC
- **VPS** (DigitalOcean) - Control total, ~$5/mes
